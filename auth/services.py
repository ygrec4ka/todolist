...
class AuthService:
    ...
    async def logout_user(
        ...
        """
        Верифицируем (декодируем) refresh token чтобы получить jti
        """
        if refresh_token:
            try:
                payload = jwt_manager.verify_refresh_token(refresh_token)
    ...
    async def logout_all_devices(
        ...
        """
        Logout со всех устройств - помечаем все refresh токены пользователя как отозванные
        """
        stmt = select(RefreshToken).where(
        ...
        # Помечаем все токены пользователя как отозванные и коммитим
